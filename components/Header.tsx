...
export default function Header() {
  ...
  const [menuOpen, setMenuOpen] = useState(false); // For Profile Dropdown
  const [mobileNavOpen, setMobileNavOpen] = useState(false); // For Mobile Hamburger
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[rgba(22,22,22,0.6)] backdrop-blur-[12px] border-b border-white/[0.05]">
      <div className="w-full px-4 sm:px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="md:hidden flex items-center justify-center p-2 text-slate-300 hover:text-white focus:outline-none"
      ...
      {mobileNavOpen && (
        <div className="md:hidden border-t border-white/10 bg-[#141414] px-4 py-6 flex flex-col gap-4 shadow-xl">
          <Link
            href="/about"
          ...
          {!user && (
            <div className="flex flex-col gap-3 pt-4 mt-2 border-t border-white/10">
              <Link
                href="/login"
                onClick={() => setMobileNavOpen(false)}
                className="w-full text-center py-3 rounded-lg text-sm font-medium text-slate-300 hover:text-white hover:bg-white/5 transition-colors"
              >
                Log in
              </Link>
              <Link
                href="/signup"
                onClick={() => setMobileNavOpen(false)}
                className="w-full text-center py-3 rounded-lg text-sm font-bold text-black bg-white transition-all active:scale-95"
              >
                Sign up
              </Link>
            </div>
