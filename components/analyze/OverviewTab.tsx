...
export default function OverviewTab({ data }: { data: RepoData }) {
  const grade = data?.analysis?.health_status?.grade;
  const score = data?.analysis?.health_status?.score;
  const status = data?.analysis?.health_status?.status;
  const refactorPlan = data?.analysis?.health_status?.refactor_plan;
  const blastRadius = data?.analysis?.blast_radius;
  const keyModules = data?.analysis?.key_modules;
  const techStack = data?.analysis?.tech_stack;
  const executionFlow = data?.analysis?.execution_flow;
  const onboardingGuide = data?.analysis?.onboarding_guide;

  const gradeColorMap: Record<string, string> = {
    A: "bg-emerald-500/10 border-emerald-500/30 text-emerald-400",
    B: "bg-blue-500/10 border-blue-500/30 text-blue-400",
    C: "bg-amber-500/10 border-amber-500/30 text-amber-400",
  ...
  const gradeCardClass =
    grade && gradeColorMap[grade]
      ? gradeColorMap[grade]
      : "bg-red-500/10 border-red-500/30 text-red-400";

  const gradeGlowClass =
    grade && gradeGlowMap[grade] ? gradeGlowMap[grade] : "bg-red-500";
      ...
      >
        <div className="xl:col-span-8">
          {data?.analysis?.health_status && (
            <div className="glass-card relative overflow-hidden p-6 sm:p-8 rounded-2xl border border-white/10 bg-[#0e0e0e] h-full transition-all">
              <div
                className={`absolute -right-20 -top-20 w-64 h-64 blur-[100px] rounded-full opacity-20 pointer-events-none ${gradeGlowClass}`}
              />
              <div className="flex flex-col md:flex-row gap-8 items-center md:items-start relative z-10 text-center md:text-left">
                <motion.div
                  ...
                  className={`flex-shrink-0 w-32 h-32 rounded-3xl flex flex-col items-center justify-center border-2 cursor-default shadow-[0_0_30px_rgba(0,0,0,0.5)] ${gradeCardClass}`}
                >
                  <motion.span
                    style={{ translateZ: 30 }}
        ...
        <div className="xl:col-span-4 h-full">
          <div className="glass-card p-6 rounded-2xl border border-white/10 bg-[#0e0e0e] h-full flex flex-col space-y-6">
            {Array.isArray(blastRadius) && blastRadius.length > 0 && (
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          <div className="glass-card p-6 rounded-2xl border border-white/5 bg-[#0e0e0e]">
            ...
            <div className="space-y-3">
              {Array.isArray(onboardingGuide) &&
                onboardingGuide.map((tip, i) => (
                  <div
                    key={i}
