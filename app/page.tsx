...
// --- PRINCIPAL UPGRADE: LazyMotion + `m` for tree-shaken animation bundle ---
// `m` is a lightweight (~4.6kb) substitute for `motion` that defers loading the
// full animation engine until <LazyMotion> hydrates `domAnimation` features.
import {
  LazyMotion,
  domMax,
  m,
...
const capabilities = [
  {
    ...
    description:
      "Maps imports, fan-in, fan-out, and file relationships so teams can see the shape of a repo before touching it.",
  },
  {
    icon: Brain,
    label: "Intelligence",
    ...
    description:
      "Summarizes entry points, patterns, risk areas, and responsibilities without forcing engineers through every file.",
  },
  {
    icon: Workflow,
    label: "Flow",
    ...
    description:
      "Connects routes, services, utilities, and stateful modules into readable flows for onboarding and debugging.",
  },
  {
    icon: Shield,
    label: "Risk",
    ...
    description:
      "Shows what breaks when a key file changes, helping teams prioritize review, tests, and migration work.",
  },
  {
    icon: Gauge,
    label: "Quality",
    ...
    description:
      "Highlights dense modules, fragile dependencies, and files that need refactoring before they become bottlenecks.",
  },
  {
    icon: Fingerprint,
    label: "Context",
    ...
    description:
      "Finds route handlers, app roots, config boundaries, and public APIs so the first read starts in the right place.",
  },
] as const;
...
// --- PRINCIPAL UPGRADE: Hydration-Safe Counter with Reduced Motion Support ---
// 1. `useReducedMotion()` -> respects WCAG 2.3.3 (Animation from Interactions).
// 2. Animation controls are explicitly stopped on unmount, killing any
//    background ticks if the user backgrounds the tab mid-animation.
// 3. The `cancelled` flag prevents stale DOM writes after unmount.
const Counter = ({
  value,
  duration = 2,
}: {
  ...
  useEffect(() => {
    ...
    // Reduced-motion fallback: snap straight to the final value.
...
function SectionHeader({
  ...
  return (
    ...
    >
      ...
      >
        {/* We replaced <Sparkles /> with the dynamic <Icon /> */}
...
export default function Home() {
  ...
  // --- PRINCIPAL UPGRADE: Navigation State (Ghost-Click Defense) ---
  const [isNavigating, setIsNavigating] = useState(false);

  // --- PRINCIPAL UPGRADE: Stable ID for label/input association ---
  const repoInputId = useId();

  // --- PRINCIPAL UPGRADE: Memory-Safe Effect (AbortController + isMounted) ---
  // 1. `fetch` is cancelled via AbortController if the component unmounts mid-flight.
  // 2. `getSystemTelemetry` is a Server Action — not abortable at the transport
  //    layer (RSC limitation), so we guard the setState with isMounted.
  // 3. AbortError is silently swallowed; everything else is logged for ops.
  useEffect(() => {
    let isMounted = true;
    ...
    getSystemTelemetry()
      ...
      .catch((err) => {
        if (!isMounted) return;
        // Silent failure — telemetry is non-critical UI surface.
  ...
  const handleAnalyze = () => {
    // --- PRINCIPAL UPGRADE: Ghost-click guard ---
  ...
  return (
    // --- PRINCIPAL UPGRADE: LazyMotion boundary ---
    // Defers loading the ~30kb animation engine until after first paint.
    // All `m.*` components inside this tree share the lazy-loaded feature set.
        ...
        <section className="relative pt-32 pb-16 md:pt-40 md:pb-24 px-4 sm:px-6 z-10">
          <div className="max-w-6xl mx-auto">
            ...
            >
              ...
              >
                CodeAutopsy turns unfamiliar repositories into architecture
                maps, dependency insights, execution flows, and prioritized
                engineering notes in minutes.
              </m.p>
              ...
              >
                {/* --- PRINCIPAL UPGRADE: Visually-hidden semantic label --- */}
                ...
                <p className="text-center text-slate-500 text-xs sm:text-sm flex flex-wrap justify-center gap-2">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className="text-slate-400 hover:text-white transition-colors text-xs sm:text-sm font-mono hover:bg-white/[0.05] px-2 py-0.5 rounded disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {ex}
                    </button>
                  ))}
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a]/90 p-8 sm:p-12 overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10 text-center mb-12">
                  ...
                  <p className="mt-4 text-base text-slate-400">
                    The homepage starts with the analyzer because that is the
                    job.
                  </p>
                </div>
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              ...
              <p className="text-base sm:text-lg text-slate-400 mb-10 max-w-2xl mx-auto px-4">
                Paste a GitHub URL, upload local code, and get the first useful
                read of the system without losing an afternoon.
              </p>
              <m.button
