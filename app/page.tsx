...
export default function Home() {
  ...
  const features = [
    ...
    { icon: Users, title: "Onboarding", description: "Join a new team and contribute on day one. Understand the stack without 50 hours of walkthroughs." },
    { icon: Zap, title: "Hackers", description: "Quickly evaluate starter repos and boilerplates. Spend your time building, not reading docs." },
    { icon: Search, title: "Recruiters", description: "Review candidate GitHub profiles efficiently. Verify code quality and project complexity in minutes." },
  ];

  const steps = [
    { icon: Link2, title: "1. Paste Repo URL", description: "Simply paste any public GitHub repository link. No setup required." },
    { icon: Cpu, title: "2. AI Analyzes Code", description: "Our AI maps dependencies, entry points, and high-level logic in seconds." },
    { icon: Map, title: "3. Explore Architecture", description: "Interact with visual diagrams and understand the logic flow instantly." },
  ];
  ...
  const pricingPlans = [
    {
      ...
      price: "$0",
      period: "/forever",
      features: ["5 Public Repos / month", "Gemini 1.5 Flash", "Basic Architecture Maps"],
      cta: "Get Started",
      isPrimary: true,
      status: "active"
    },
    {
      ...
      price: "$19",
      period: "/mo",
      features: ["Private Repositories", "Gemini 1.5 Pro", "Export to Markdown"],
      cta: "Locked",
      isPrimary: false,
      status: "coming-soon",
      badge: "Coming Soon"
    },
    {
      ...
      price: "Custom",
      period: "",
      features: ["Unlimited Analysis", "Team Collaboration", "Custom AI Models"],
      cta: "Waitlist",
      isPrimary: false,
      status: "coming-soon",
      badge: "Coming Soon"
    ...
    <div className="min-h-screen relative overflow-x-hidden bg-gradient-to-b from-[#0e0e0e] via-[#0e0e0e] to-[#1a1a1a]">
      ...
      >
        <div className="max-w-4xl mx-auto text-center">
          ...
          <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/[0.03] border border-white/10 text-slate-400 text-xs font-bold mb-8 uppercase tracking-widest">
          ...
          <motion.p variants={fadeUp} className="text-lg md:text-xl text-slate-400 mb-10 max-w-2xl mx-auto leading-relaxed">
            Stop wrestling with unfamiliar code. CodeAutopsy analyzes GitHub repositories to provide visual architecture, execution flows, and AI-driven insights.
          </motion.p>
  ...
  <div key={name} className="flex items-center gap-3 text-2xl font-bold text-slate-400">
    ...
    <span>{name}</span>
      ...
      <section id="how-it-works" className="py-32 px-6">
        ...
        >
          <motion.div variants={fadeUp} className="text-center mb-20">
            <h2 className="text-4xl font-bold mb-4 text-slate-50">The Autopsy Process</h2>
            <p className="text-slate-400">Three steps to complete architectural clarity.</p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {steps.map((step, idx) => {
              const Icon = step.icon;
              return (
                <motion.div variants={fadeUp} key={idx} className="flex flex-col items-center text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <motion.div variants={fadeUp}>
              ...
              <div className="space-y-6">
                {[
                  { title: "Architecture Diagrams", desc: "Auto-generated maps of folder structures and module relationships." },
                  { title: "Interactive Q&A", desc: 'Ask things like "Where is the authentication logic?" or "How are API calls handled?"' },
                  { title: "Execution Flow Mapping", desc: "Trace the journey of a request from the UI down to the database level." },
                ].map((item, idx) => (
                  <div key={idx} className="flex gap-4">
                    <Check className="w-6 h-6 text-slate-400 mt-1 flex-shrink-0" />
                    <div>
                      <h4 className="font-bold mb-1 text-slate-100">{item.title}</h4>
            ...
            <motion.div variants={fadeUp} className="glass-card rounded-2xl overflow-hidden border-white/10">
              ...
              <div className="p-6 space-y-4">
                {["Architecture: Library", "Entry: index.js", "Files analyzed: 206", "Tech stack: Node.js, Express"].map((line, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-slate-600" />
                    <span className="text-sm font-mono text-slate-500">{line}</span>
                  </div>
                ))}
      ...
      <section className="py-32 px-6">
        ...
        >
          <motion.div variants={fadeUp} className="text-center mb-16">
            <h2 className="text-4xl font-bold mb-4 text-slate-50">Tailored for the Ecosystem</h2>
            <p className="text-slate-400">Solutions for students, developers, and talent hunters.</p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature, idx) => {
              const Icon = feature.icon;
              return (
                <motion.div variants={fadeUp} key={idx} className="glass-card p-8 rounded-2xl hover:bg-white/[0.02] transition-all">
      ...
      <section id="pricing" className="py-32 bg-black/40 px-6">
        ...
        >
          <motion.div variants={fadeUp} className="text-center mb-16">
            <h2 className="text-4xl font-bold mb-4 text-slate-50">Simple, Transparent Pricing</h2>
            <p className="text-slate-400">Scale as you explore more repositories.</p>
          </motion.div>
            ...
            {pricingPlans.map((plan, idx) => (
              <motion.div
                ...
                className={`glass-card p-10 rounded-2xl flex flex-col relative transition-all ${
                  plan.isPrimary ? "border-white/10 gray-soft-glow" : "border-white/5"
              ...
              >
                ...
                <div className="mb-8">
                  <h3 className={`text-lg font-bold mb-2 ${plan.status === 'coming-soon' ? "text-slate-500 italic" : "text-slate-100"}`}>
                    {plan.name}
                  </h3>
                  <div className="flex items-baseline gap-1">
                    <span className={`text-4xl font-bold ${plan.status === 'coming-soon' ? "text-slate-600" : "text-white"}`}>
                      {plan.price}
                    </span>
                ...
                <ul className={`space-y-4 mb-10 flex-1 ${plan.status === 'coming-soon' ? "text-slate-600" : "text-slate-400"}`}>
      ...
      <footer className="py-20 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div>
              ...
              <p className="text-slate-400 text-sm leading-relaxed max-w-xs">
                The world's first AI-powered codebase visualization tool. Built for efficiency.
              </p>
            </div>
            {[
              { 
              ...
              <div key={col.title}>
                <h4 className="font-bold mb-6 text-xs uppercase tracking-[0.2em] text-slate-400">{col.title}</h4>
                <ul className="space-y-4">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <Link href={link.href} className="cursor-pointer text-slate-400 hover:text-white text-sm transition-colors">
                        {link.label}
                      </Link>
                    </li>
                  ))}
            ...
            <div>
              <h4 className="font-bold mb-6 text-xs uppercase tracking-[0.2em] text-slate-400">Connect</h4>
              <div className="flex gap-4">
                {[
                  ...
                  return (
                    <a key={i} href={social.href} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-lg bg-[#141414] border border-white/5 flex items-center justify-center text-slate-400 hover:text-white hover:border-white/20 transition-all">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-white/5 gap-4">
            <p className="text-slate-600 text-[11px] font-medium tracking-tight uppercase">© 2026 CodeAutopsy Inc. All rights reserved.</p>
            <p className="text-slate-600 text-[11px] font-medium tracking-tight uppercase">Engineered for the developer community</p>
          </div>
        </div>
      </footer>
